...
## 8. Make `ImportEntityZip` faster: bypass inline base64 transport

**Problem:** `ImportEntityZip`'s `zipContent` parameter takes the zip as an inline base64 `STRING`. Whatever MCP client calls it — Claude included — has to produce that entire string as literal output before the call can even be dispatched, since tool-call arguments are always authored directly, not referenced or forwarded from elsewhere in context. For an 11-file, ~16KB zip this is already a multi-minute, thousands-of-output-token cost; it only gets worse as the entity set grows. Worse than the slowness: reproducing a ~16K-character high-entropy string as generated output has a real, observed failure mode — a live test in this repo hit exactly this, where the reproduced base64 came out corrupted and the import failed with `zip END header not found`, even though the source zip was valid. Neither a file-based hand-off, a script printing the blob to stdout, nor a clipboard-paste-into-chat workaround fixes this — all of them still require the calling model to re-emit the full string as the tool call's argument; only removing inline content from the transport does.

**Direction:** have the packaging step upload the zip directly to `SystemRepository` (or wherever it needs to land) over HTTP from the client machine — bypassing the MCP tool as the transport entirely — then give `ImportEntityZip` (or a new variant of it) a mode that takes a server-side file path/reference instead of inline content. No base64, no model-generated payload, no reproduction-corruption risk.

**Open question to resolve before building this:** does `SystemRepository` (or another ThingWorx resource) expose a REST endpoint suitable for a script to upload a file to directly, without going through `SaveBinary` via an MCP call? If yes, this is a straightforward addition. If not, this may require a small custom REST service on the ThingWorx side first.